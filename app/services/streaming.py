...
def _collect_st_memory(session_id: str, turns: int = 6) -> str:
    ...
    for m in tail:
        ...
        lines.append(f"{role.UPPER() if hasattr(role,'UPPER') else role.upper()}: {text}")
    return "\n".join(lines)
...
def make_assistant_frames(seed_text: str, session_id: str | None = None, meta: dict | None = None) -> Tuple[str, List[Dict]]:
    ...
    # TTS (MP3 + visemes) — gated by feature_audio
    audio_on = cfg_db.get("feature_audio", True)
    a_bytes, vis = (b"", [])
    if audio_on:
