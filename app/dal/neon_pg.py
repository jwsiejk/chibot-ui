...
def _connect():
    ...
    Supports:
      ...
      - postgresql://user:pass@host/db?sslmode=require  (Neon pooler)
    """
    global _DB, _DIALECT
    if _DB:
        return _DB
    ...
    if url.startswith("sqlite:"):
        ...
        if path.startswith("/"):
            # sqlite:////mnt/data/app.sqlite -> //mnt/data/app.sqlite
            path = path[1:]
        _DIALECT = "sqlite"
...
def ensure_schema():
    if _DIALECT is None:
        _connect()
    # SQLite uses ?, Postgres uses %s — our DDL has no placeholders, safe on both
    for stmt in DDL:
        _exec(stmt)
    return True
...
def save_config(cfg: Dict[str, Any]) -> int:
    ensure_schema()
    _exec("INSERT INTO admin_settings (cfg_json, created_at) VALUES (%s, %s)" if _DIALECT=="postgresql" else
          "INSERT INTO admin_settings (cfg_json, created_at) VALUES (?, ?)",
...
def save_layout(breakpoint: str, state: Dict[str, Any], note: str = "") -> int:
    ensure_schema()
    rows = _exec("SELECT COALESCE(MAX(version),0) FROM layouts WHERE breakpoint=%s" if _DIALECT=="postgresql" else
                 "SELECT COALESCE(MAX(version),0) FROM layouts WHERE breakpoint=?", [breakpoint], fetch=True)
    v = int(rows[0][0]) + 1 if rows else 1
    _exec("INSERT INTO layouts (breakpoint, version, state_json, note, created_at) VALUES (%s,%s,%s,%s,%s)" if _DIALECT=="postgresql" else
          "INSERT INTO layouts (breakpoint, version, state_json, note, created_at) VALUES (?,?,?,?,?)",
          [breakpoint, v, json.dumps(state), note or "", time.time()])
    return v

def list_layouts(breakpoint: str) -> List[Dict[str, Any]]:
    ensure_schema()
    rows = _exec("SELECT version, state_json, note, created_at FROM layouts WHERE breakpoint=%s ORDER BY version ASC" if _DIALECT=="postgresql" else
                 "SELECT version, state_json, note, created_at FROM layouts WHERE breakpoint=? ORDER BY version ASC",
...
def get_layout(breakpoint: str, version: int) -> Dict[str, Any]:
    ensure_schema()
    rows = _exec("SELECT state_json FROM layouts WHERE breakpoint=%s AND version=%s" if _DIALECT=="postgresql" else
                 "SELECT state_json FROM layouts WHERE breakpoint=? AND version=?", [breakpoint, version], fetch=True)
    if not rows:
        return {}
...
def upsert_user(email: str, name: str = "", title: str = "", region: str = ""):
    ...
    if _DIALECT=="postgresql":
        _exec(
            """INSERT INTO users (email, name, title, region, created_at, last_seen)
               VALUES (%s,%s,%s,%s,%s,%s)
               ON CONFLICT (email) DO UPDATE SET last_seen=EXCLUDED.last_seen""",
            [email, name, title, region, now, now]
        )
    else:
        # SQLite 3.24+ supports UPSERT with DO UPDATE
        _exec(
            """INSERT INTO users (email, name, title, region, created_at, last_seen)
               VALUES (?, ?, ?, ?, ?, ?)
               ON CONFLICT(email) DO UPDATE SET last_seen=excluded.last_seen""",
...
def load_profile(email: str) -> Dict[str, Any]:
    ensure_schema()
    rows = _exec("SELECT profile_json FROM profiles WHERE email=%s" if _DIALECT=="postgresql" else
                 "SELECT profile_json FROM profiles WHERE email=?", [email], fetch=True)
    if not rows:
        return {}
...
def ensure_session(session_id: str, email: str, persona_id: str = "chip"):
    ensure_schema()
    rows = _exec("SELECT 1 FROM sessions WHERE id=%s" if _DIALECT=="postgresql" else
                 "SELECT 1 FROM sessions WHERE id=?", [session_id], fetch=True)
    if not rows:
        _exec("INSERT INTO sessions (id, email, persona_id, started_at, ended_at, summary_json) VALUES (%s,%s,%s,%s,%s,%s)" if _DIALECT=="postgresql" else
              "INSERT INTO sessions (id, email, persona_id, started_at, ended_at, summary_json) VALUES (?,?,?,?,?,?)",
              [session_id, email, persona_id, time.time(), None, "{}"])

def add_message(session_id: str, role: str, text: str, meta: Dict[str, Any] = None):
    ensure_schema()
    _exec("INSERT INTO messages (session_id, role, text, meta_json, created_at) VALUES (%s,%s,%s,%s,%s)" if _DIALECT=="postgresql" else
          "INSERT INTO messages (session_id, role, text, meta_json, created_at) VALUES (?,?,?,?,?)",
...
def list_sessions(email: str = None) -> List[Dict[str, Any]]:
    ensure_schema()
    if email:
        rows = _exec("SELECT id, email, persona_id, started_at, ended_at, summary_json FROM sessions WHERE email=%s ORDER BY started_at DESC" if _DIALECT=="postgresql" else
                     "SELECT id, email, persona_id, started_at, ended_at, summary_json FROM sessions WHERE email=? ORDER BY started_at DESC",
                     [email], fetch=True)
    else:
    ...
    for r in rows or []:
        if _DIALECT=="sqlite":
            ...
            out.append({"id": r[0], "email": r[1], "persona_id": r[2],
...
def list_messages(session_id: str) -> List[Dict[str, Any]]:
    ensure_schema()
    rows = _exec("SELECT role, text, created_at FROM messages WHERE session_id=%s ORDER BY id ASC" if _DIALECT=="postgresql" else
                 "SELECT role, text, created_at FROM messages WHERE session_id=? ORDER BY id ASC", [session_id], fetch=True)
    out = []
    for r in rows or []:
        if _DIALECT=="sqlite":
            out.append(dict(r))
...
def anonymize_session(session_id: str):
    ensure_schema()
    _exec("UPDATE sessions SET email='anonymized@example.com' WHERE id=%s" if _DIALECT=="postgresql" else
          "UPDATE sessions SET email='anonymized@example.com' WHERE id=?",
          [session_id])
...
def load_all_into_memory(memory: Dict[str, Any]):
    ...
    try:
        ...
        for r in rows or []:
            bp = r[0] if _DIALECT=="postgresql" else r["breakpoint"]
            mv = int(r[1] if _DIALECT=="postgresql" else r["MAX(version)"] or 0)
            if mv:
