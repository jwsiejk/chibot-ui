...
class DB:
    ...
    def ensure_session(self, sid, email): self.memory['sessions'].setdefault(sid,{'email':email,'messages':[],'nudges':0,'persona_id':'chip'})
    def add_message(self, sid, role, text): self.memory['sessions'].setdefault(sid,{'email':'user@example.com','messages':[],'nudges':0,'persona_id':'chip'}); self.memory['sessions'][sid]['messages'].append((role,text))
    ...
    def update_config(self, updates): self.memory['configs'].update(updates); return self.get_config()
