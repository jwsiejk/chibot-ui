...
def _asgi_client():
    try:
        ...
        return None
    for n in ("asgi","app","application"):
        app = getattr(gw, n, None)
        if app is not None:
            try:
...
def _ensure_cookie(client, token):
    # Starlette requests client jar
    try:
        jar = getattr(client, "cookies", None)
        if jar is not None:
            try: jar.set("csrf", token, domain="testserver", path="/")
            except Exception: pass
        # Flask fallback
        if hasattr(client, "set_cookie"):
            try: client.set_cookie("testserver", "csrf", token, path="/")
            except Exception: pass
    except Exception:
        pass
