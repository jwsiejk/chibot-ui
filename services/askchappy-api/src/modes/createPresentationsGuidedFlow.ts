...
export const handleCreatePresentationsTurn = async (session: AskChappySession): Promise<GuidedAssistantResponse> => {
  ...
  if (ddn.stage==='use_case') {
    ...
    if (!ddn.useCase && !v) return ask('Choose a customer use case:\n\n1. AI / GenAI infrastructure\n2. Life sciences research and genomics\n3. Healthcare imaging and clinical data\n4. HPC / research computing\n5. Data analytics at scale\n6. Private AI / secure AI platform\n7. Cloud service provider / AI cloud','Choose the customer use case.');
    ddn.useCase=(ddn.useCase??v) as DdnUseCase; ddn.stage='audience';
  }
  if (ddn.stage==='product') { const v=parse(userText,{infinia:'1|one|infinia',ai400x:'2|two|ai400x|ai400x2|ai400x3',exascaler:'3|three|exascaler',dip:'4|four|data intelligence platform|dip',intelliflash:'5|five|intelliflash',insight:'6|six|insight|data intelligence',recommend:'7|seven|recommend|best fit'}); if(!ddn.product&&!v)return ask('Choose the DDN product or solution focus:\n\n1. Infinia\n2. AI400X / AI400X2 / AI400X3\n3. EXAScaler\n4. Data Intelligence Platform\n5. IntelliFlash\n6. Insight / data intelligence\n7. Recommend the best fit','Choose the DDN solution focus.'); ddn.product=(ddn.product??v) as DdnProductFocus; if(ddn.product!=='recommend') ddn.useCase ??= inferUseCaseFromProduct(ddn.product); ddn.stage=ddn.product==='recommend'?'recommend':'audience'; }
  if (ddn.stage==='recommend'){const v=parse(userText,{executive:'1|executive ai/data strategy',life:'2|life sciences|genomics',technical:'3|technical architecture',partner:'4|partner enablement',hpc:'5|hpc|research computing',general:'6|general ddn overview|not sure'}); if(!v)return ask('What kind of customer conversation is this?\n\n1. Executive AI/data strategy\n2. Life sciences / genomics\n3. Technical architecture\n4. Partner enablement\n5. HPC / research computing\n6. General DDN overview','Choose the conversation type.'); ddn.useCase=(v==='life'||v==='general')?'life_sciences_genomics':v==='hpc'?'hpc_research_computing':'ai_genai_infrastructure'; ddn.product='recommend'; ddn.audience ??='executive'; ddn.depth ??='standard'; ddn.stage='audience'; }
  if (ddn.stage==='audience'){const v=parse(userText,{executive:'1|executive|cio|cto',infrastructure:'2|infrastructure leadership',ai_data_platform:'3|ai / data platform team|ai data platform',research_scientific:'4|research|scientific computing',storage_hpc:'5|storage|hpc technical',partner_enablement:'6|partner sales|se enablement'}); if(!ddn.audience&&!v)return ask('Who is the audience?\n\n1. Executive / CIO / CTO\n2. Infrastructure leadership\n3. AI / data platform team\n4. Research / scientific computing team\n5. Storage / HPC technical team\n6. Partner sales / SE enablement','Choose the audience.'); ddn.audience=(ddn.audience??v) as DdnAudience; ddn.stage='customer';}
  if (ddn.stage==='customer'){ if(!ddn.customer&&!userText) return ask('Customer or account name?\nType the name, or choose 1 to skip.','Add the customer name, or choose skip.'); ddn.customer = (n(userText)==='1'||n(userText)==='skip')?'Skipped':userText; ddn.stage='depth'; }
  if (ddn.stage==='depth'){const v=parse(userText,{short_exec:'1|short executive deck',standard:'2|standard customer meeting',technical:'3|technical detail included'}); if(!ddn.depth&&!v)return ask('How deep should this deck go?\n\n1. Short executive deck\n2. Standard customer meeting\n3. Technical detail included','Choose the deck depth.'); ddn.depth=(ddn.depth??v) as DdnDepth; ddn.stage='notes';}
  if (ddn.stage==='notes'){ if(!ddn.notes&&!userText) return ask('Anything specific to include?\nType notes, or choose 1 to skip.','Add optional notes, or choose skip.'); ddn.notes=(n(userText)==='1'||n(userText)==='skip')?'Skipped':userText; ddn.stage='review'; }
  ...
  if (ddn.stage==='edit'){ const c=n(userText); if(c==='1'){ddn.stage='starting';ddn.startingPoint=undefined;} else if(c==='2'){ddn.stage='use_case';ddn.useCase=undefined;} else if(c==='3'){ddn.stage='product';ddn.product=undefined;} else if(c==='4'){ddn.stage='audience';ddn.audience=undefined;} else if(c==='5'){ddn.stage='customer';ddn.customer=undefined;} else if(c==='6'){ddn.stage='depth';ddn.depth=undefined;} else if(c==='7'){ddn.stage='notes';ddn.notes=undefined;} return ask('Updated. Let’s continue your DDN deck setup.','Updated. Let’s continue.'); }
  ...
  const generatePptxRuntime = async (): Promise<PptxGenerationResult> => { const { generatePptxFromApprovedOutline } = await import('./createPresentationsPptxGenerator'); return generatePptxFromApprovedOutline(session.session_id, state.deckBrief, state.outline); };
