...
describe('askchappy-api scaffold', () => {
  ...
  it('routes create_presentations user turns through guided interview and supports approval', async () => {
    ...
    expect(reviewMsg?.text).toContain('1. This is correct — generate the outline');
    expect(reviewMsg?.text).toContain('2. Edit the brief');
    appendLocalUserTextMessage(session.session_id, '1');
    await generateLocalAssistantMessage(session.session_id);
    const outlineMsg = getLocalTranscript(session.session_id).at(-1);
    expect(outlineMsg?.text).toContain('Great — I approved the brief and generated the outline');
    expect(outlineMsg?.text).toContain('Deck Outline Review');
    const updated = getLocalSession(session.session_id);
    expect(updated?.metadata.askchappy.create_presentations_state?.step).toBe('outline_review');
    expect(updated?.metadata.askchappy.create_presentations_state?.deckBrief.status).toBe('outline_review');
    expect(updated?.metadata.askchappy.create_presentations_state?.outline.status).toBe('outline_review');
  });
});
