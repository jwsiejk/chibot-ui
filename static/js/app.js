...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', async ()=>{
    ...
    if(Lf){ Lf.addEventListener('submit', async (e)=>{ e.preventDefault(); const email=(Lem?.value||'').trim(); if(!email){ Lmsg.textContent='Please enter a valid email.'; return; } Lbtn.disabled=true; Lmsg.textContent='Signing in…'; showLogin(false); try{ const tok=await ensureCSRF(); const r=await fetch('/api/v1/auth/login',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json',...(tok?{'X-CSRF-Token':tok}:{})},body:JSON.stringify({email})}); const me=await fetch('/api/v1/auth/me',{credentials:'include'}).then(x=>x.json()).catch(()=>null); if(r.ok && me && me.authenticated){ if(me.profile_complete===false){ await prefillProfile(); showProfile(true); } else { showProfile(false); } await refreshGating(); } else { Lmsg.textContent='Login failed. Please try again.'; showLogin(true); Lbtn.disabled=false; } }catch(_){ Lmsg.textContent='Network error. Please try again.'; showLogin(true); Lbtn.disabled=false; } }); }
    ...
    if(Pf){ Pf.addEventListener('submit', async (e)=>{ e.preventDefault(); const data={ email:(el('prof_email')?.value||'').trim(), name:(el('prof_name')?.value||'').trim(), role:(el('prof_role')?.value||'').trim(), region:(el('prof_region')?.value||'').trim(), company:(el('prof_company')?.value||'').trim(), completed:true }; if(!data.name||!data.role||!data.region){ Pmsg.textContent='Please complete name, title, and region.'; return; } Pbtn.disabled=true; Pmsg.textContent='Saving…'; try{ const tok=await ensureCSRF(); const r=await fetch('/api/v1/auth/profile/save',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json',...(tok?{'X-CSRF-Token':tok}:{})},body:JSON.stringify(data)}); if(r.ok){ showProfile(false); await refreshGating(); } else { Pmsg.textContent='Save failed.'; } }catch(_){ Pmsg.textContent='Network error.'; } finally{ Pbtn.disabled=false; } }); }
  });
})();
