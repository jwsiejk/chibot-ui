...
function setDot(state){ const d=$('#stateDot'); if(!d) return; d.className='dot '+(state==='listening'?'dot-listening': state==='speaking'?'dot-speaking': state==='thinking'?'dot-thinking':'dot-ready'); $('#statusText')?.textContent = state.charAt(0).toUpperCase()+state.slice(1); }
