...
export async function start(){
  if (_rec) return;
  if (!_cfg.getStream) throw new Error('recorder-lite: getStream() not provided');
  const s = _cfg.getStream();
  if (!s) throw new Error('recorder-lite: no mic stream; call VAD.arm() first');
  _chunks=[];
  const opts={ mimeType:_cfg.mimeType };
  _rec = new MediaRecorder(s, opts);
  _rec.ondataavailable = (e)=>{ if (e.data && e.data.size) _chunks.push(e.data); };
  _rec.onstop = async ()=>{ const blob=new Blob(_chunks,{type:'audio/webm'}); _rec=null; try{ await _cfg.onBlob?.(blob); }catch(e){ console.warn('onBlob error',e); } };
  _rec.start();
}
