...
/**
 * send.js — New API only (soft/echo‑aware barge‑in) + assistant events + requestNudge()
 */
...
export async function handleVoiceOnceResponse(evtOrMsg){
  ...
  if (evtOrMsg && 'data' in evtOrMsg){
    ...
    if (typeof evtOrMsg.data === "string"){ try{ msg=JSON.parse(evtOrMsg.data);}catch{msg=null;} }
      ...
      let buf; if (evtOrMsg.data instanceof ArrayBuffer) buf = evtOrMsg.data; else if (evtOrMsg.data instanceof Blob) buf = await evtOrMsg.data.arrayBuffer();
      if (buf){ if (!_assistantSpeaking){ _assistantSpeaking=true; try{ window.dispatchEvent(new CustomEvent('chip:assistant_speaking')); }catch{} } barge?.onAssistantAudioStart(); (ttsPlayer || (ttsPlayer=createFallbackTTSPlayer())).appendChunk(buf); return; }
    }
  ...
  switch (msg.type){
    ...
    case "suggestions": try { window.dispatchEvent(new CustomEvent("chip:suggestions", { detail: msg.items || [] })); } catch {} break;
    case "audio_chunk": {
