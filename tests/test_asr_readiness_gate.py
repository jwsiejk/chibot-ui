...
class TestASRReadinessGate(unittest.TestCase):
    ...
    async def _exercise_asr_concurrency(self) -> tuple[List[dict], Dict[str, Any]]:
        adapter = ChatV2Adapter()
        engine = RecordingEngine()
        adapter.engine = engine
    ...
    def test_binary_post_ready_accepted(self) -> None:
        ...
        self.assertEqual(ready_frame.get("vendor"), "deepgram")
    ...
    @patch.object(ChatV2Adapter, "_allowed_asr_vendors", return_value=["deepgram"])
    def test_deepgram_concurrency_sets_grace(self, _mock_allowed: Any) -> None:
        sent, snapshot = asyncio.run(self._exercise_asr_concurrency())

        self.assertEqual(snapshot["vendor"], "deepgram")
