...
- During those VMware PATCH-time log-sufficiency refreshes, AskChip may also recompute and persist deterministic policy fields (`policy_next_move`, policy-aligned `conversation_stage`, and `next_best_question`) from the current typed triage state.
