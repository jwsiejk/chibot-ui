...
export const ChappySession = () => {
  ...
  const session = useMemo(() => (sessionId ? serverApi.getLocalSession(sessionId) : undefined), [sessionId, version]);
  const voiceStatus = useMemo(() => serverApi.getLocalVoiceStatus(), []);
  const messages: TranscriptMessage[] = useMemo(() => (!sessionId || !session ? [] : serverApi.getLocalTranscript(sessionId)), [sessionId, session, version]);
  ...
  const speakAssistant = async (
    ...
    const tts = await serverApi.synthesizeLocalAssistantMessage(sessionId, messageId);
  ...
  const runAssistantTurn = async (turnStartAt: number, submitAt: number, turnType: 'typed' | 'voice', micCaptureMs: number | null, sttMs: number | null) => {
    ...
    setState('thinking');
    const result = await serverApi.generateLocalAssistantMessage(sessionId);
    ...
    setVersion((v) => v + 1);
    const refreshed = serverApi.getLocalTranscript(sessionId);
  ...
  const onSubmitText = async (text: string) => {
    ...
    serverApi.appendLocalUserTextMessage(sessionId, text);
    pushDiagnostic('typed message submitted');
    setVersion((v) => v + 1);
    await runAssistantTurn(turnStartAt, turnStartAt, 'typed', null, null);
  };

  const onSelectMode = (mode: SessionMode) => { serverApi.setLocalSessionMode(sessionId, mode, 'user'); setVersion((previous) => previous + 1); };
  const onTranscribeVoice = async (blob: Blob) => {
    ...
    const stt = await serverApi.transcribeLocalVoiceInput(sessionId, blob);
  ...
  return (
    <main className="meeting-room session-shell" aria-label="askchappy session room">
      ...
      {showModes ? <div className="modes-overlay card panel" role="dialog" aria-label="guided modes overlay"><button className="btn secondary" type="button" onClick={() => setShowModes(false)}>Close</button><SessionRightRail activeMode={session.metadata.askchappy.session_mode} generatedPresentation={session.metadata.askchappy.create_presentations_state?.generatedPresentation} generatedDeckHistory={session.metadata.askchappy.create_presentations_state?.generatedDeckHistory} onSelectMode={(mode) => { onSelectMode(mode); setShowModes(false); }} /></div> : null}
