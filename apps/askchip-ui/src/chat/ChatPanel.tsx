...
interface ChatPanelProps extends ChatPanelCoreProps {
...
function ChatPanelCore({
  ...
  onClose,
  onHeaderPointerDown,
}: ChatPanelCoreProps & { onHeaderPointerDown?: (event: ReactPointerEvent<HTMLElement>) => void }) {
  ...
  return (
    <>
      <header
        className={`flex items-center justify-between gap-3 border-b border-slate-800 bg-slate-900/80 px-4 py-3 ${onHeaderPointerDown ? 'cursor-move rounded-t-3xl' : ''}`}
...
export function ChatPanel({ mode, open, ...props }: ChatPanelProps) {
