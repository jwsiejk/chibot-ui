...
- The same PATCH-time refresh now also recomputes deterministic policy fields on persisted triage state (`policy_next_move`, policy-aligned `conversation_stage`, and `next_best_question`) so live log uploads keep VMware triage metadata coherent immediately.
