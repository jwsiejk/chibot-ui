...
- VMware AI expert live-conversation tuning now keeps post-kickoff responses shorter and more engineer-like (one focused next question, likely issue paths, and practical verification steps) while preserving explicit honesty that uploaded logs are metadata-only unless parsed content exists.
