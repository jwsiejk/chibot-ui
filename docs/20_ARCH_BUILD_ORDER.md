...
- **B5-I:** **Dual‑VAD Aggregator & Policy (production‑grade)**  
  ...
  **Behavior:** Fuse auto‑VAD and ASR evidence into one decision; enforce echo‑suppression, hysteresis, cooldown; one‑grant‑per‑TTS; per‑session isolation.  
  **Adaptation:** Track per‑session noise floor and derive an SNR‑based dynamic threshold; classify environment (quiet/normal/noisy) and adjust `mode`/`min_speech_ms` within safe bounds; bounded self‑tuning with audit.  
  **Telemetry:** Emit diagnostic `EVT_VAD` and `EVT_VAD_DECISION` (enveloped w/ `schema_version:"1"`); outcome continues via `EVT_BARGE_IN`.  
  **Acceptance:** OR/AND/priority tests; echo‑suppress; hysteresis & cooldown; isolation across sessions; adaptation increments thresholds in sustained noise and reduces in quiet, without manual tweaks.
