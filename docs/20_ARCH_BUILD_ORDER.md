...
- **B6-A:** Bundle structure & redaction (manifest, server log, ws taps, flow timeline).  
  **Acceptance:** `manifest.json` counts match streams within ±1% tolerance.
- **B6-B:** Levels/categories/sampling → policy toggles apply immediately.  
  **Acceptance:** `EVT_TELEMETRY_POLICY` shows applied diff.
- **B6-C:** Provider debug channels (IDs + timings, never secrets).  
  **Acceptance:** `EVT_VENDOR_DEBUG` present with opaque IDs only.
- **B6-D:** Admin Flow Trace API.  
  **Acceptance:** returns windowed events; respects redaction flags.
- **B6-E:** Performance Telemetry & Budgets.  
  **Acceptance:** latency histograms exported; over‑budget emits warning events.
- **B6-F:** Local Tests & Runner (Exporter/Zip/Perf API).  
  **Acceptance:** `scripts/run_build06_tests.sh` green locally.
